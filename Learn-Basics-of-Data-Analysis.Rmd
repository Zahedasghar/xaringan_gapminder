...
gapminder[1:10,] %>% filter(country=="Sweden")%>%
 mutate(gdpPercap=round(gdpPercap,0), lifeExp=round(lifeExp,2)) %>% 
kbl()%>%
...
```{r}
gapminder[1:10,] %>% select(-gdpPercap) %>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
...
```{r}
gapminder %>% filter(year==2007, country %in% c("India", "Pakistan","Bangladesh", "Afghanistan", "Iran")) %>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
```
...
gapminderSA[1:10,]%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
gapminderSA[1:10,] %>% arrange(gdpPercap)%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
...
```{r}
gapminderSA[1:10,] %>% arrange(desc(gdpPercap))%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
gapminderSA %>% filter(year==2007) %>%  arrange(lifeExp)%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
gapminderSA[1:10,] %>% mutate(pop=pop/1000000)%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
gapminderSA[1:10,] %>% mutate(gdp = pop * gdpPercap)%>% kbl() %>% kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
gapminder %>% 
  summarise(avglifeExp=mean(lifeExp))%>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
...
gapminder %>%
  ...
  summarize(avgLifeExp = mean(lifeExp)) %>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
...
gapminder %>% 
  group_by(continent) %>% 
  filter(year==2007) %>% 
  summarize(avglife=mean(lifeExp))%>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
...
gapminder %>%
  ...
  mutate(over75 = if_else(avgLifeExp > 70, "Y", "N"))%>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
...
gapminder %>% 
  filter(year==2007) %>% 
  group_by(continent) %>% 
  summarize(tot_pop=sum(pop)) %>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
...
gapminder %>% filter(year==2007) %>% 
  ...
  arrange(percentile) %>% 
  top_n(-10,wt=percentile) %>% 
  select(country,continent,lifeExp,gdpPercap)%>% kable()%>%
 kable_styling(bootstrap_options = "striped", full_width = F)
