...
survey <- read_excel("C:/Users/92300/OneDrive - Higher Education Commission/R files/Safwan_tourism.xlsx")
survey<-as.data.frame(survey)
#survey <- read.csv("survey.csv", header=TRUE)  ## load a .csv file
survey ## list a first 10 records to check your data set
```


To _read_ integers as factors
```{r}
survey1<-survey %>%mutate_if(is.numeric,as.factor)
glimpse(survey1)
```
...
fam.tab <- table(survey1$`Professional Training`, survey1$Position) ## Collects category frequencies
